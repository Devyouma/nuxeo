...
node('SLAVE') {
    ...
    stage('compile') {
        withBuildStatus('compile', 'https://github.com/nuxeo/nuxeo', sha, "${BUILD_URL}") {
            withMaven() {
                sh 'mvn -B compile -Pqa,addons -DskipTests'
            }
        }
    }
    ...
    stage('test') {
        withBuildStatus('test', 'https://github.com/nuxeo/nuxeo', sha, "${BUILD_URL}") {
            withMaven() {
                sh 'mvn -B test -Pqa,addons -Dmaven.test.failure.ignore=true -Dnuxeo.tests.random.mode=STRICT'
            }
        }
    }
    ...
    def zipfile=stage('verify') {
        withBuildStatus('verify', 'https://github.com/nuxeo/nuxeo', sha, "${BUILD_URL}") {
            withMaven() {
                sh 'mvn -B verify -Pqa,addons -DskipTests -Dmaven.test.failure.ignore=true -Dnuxeo.tests.random.mode=STRICT'
            }
        }
    ...
    stage('postgresql') {
        verifyProfile('pgsql', '9.6', zipfile)
    }

    stage('mongo') {
        verifyProfile('mongodb', '3.4', zipfile)
    }

    stage('kafka') {
        verifyProfile('kafka', '1.0', zipfile)
    }
}

def verifyProfile(String profile, String version, String zipfile) {
                ...
                ws("${WORKSPACE}-${name}") {
                    ...
                    timeout(time: 2, unit: 'HOURS') {
                        withBuildStatus("${DBPROFILE}-${DBVERSION}/ftest/${name}", 'https://github.com/nuxeo/nuxeo', sha, "${BUILD_URL}") {
                            withDockerCompose("${JOB_NAME}-${BUILD_NUMBER}-${name}", "integration/Jenkinsfiles/docker-compose-${DBPROFILE}-${DBVERSION}.yml", "mvn ${mvnopts} -B -f ${WORKSPACE}/nuxeo-distribution/${dir}/pom.xml -Pqa,tomcat,${DBPROFILE} verify", post)
                        }
                    }
                }
            }
